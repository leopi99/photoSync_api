...
func GetUserObjects(userID string) (Objects, error) {
	rows, err := database.Query("SELECT objectID, url, local_path, creation_date, sync_date, picture_position, type, byte_size, downloaded, extension, local_id FROM object WHERE userID = " + userID + ";")
	if err != nil {
		return nil, err
	}
	var objects Objects
...
func GetUserObjectsFiltered(userID string, objType string) (Objects, error) {
	rows, err := database.Query("SELECT objectID, url, local_path, creation_date, sync_date, picture_position, type, byte_size, downloaded, extension, local_id FROM object WHERE userID = " + userID + " AND type = \"" + objType + "\";")
	if err != nil {
		return nil, err
	}
	var objects Objects
